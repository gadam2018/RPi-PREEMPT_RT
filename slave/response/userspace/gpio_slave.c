...
// RUN: sudo ./gpio_slave <number-of-loops> // runs, with FIFO & priority 99
// ****************************************************************
...
#define GPIO_in 4   //means (BCM) GPIO4 is (BOARD) pin 7 
#define GPIO_out 2  // means (BCM) GPIO2 is (BOARD) pin 3  
...
/* ***************** Polling the input pin, then READ INPUT VALUE & SET OUTPUT ****************** */
...
if ((fd = open(GPIOValue_in, O_RDONLY)) < 0) //|O_NONBLOCK)) < 0) 
...
pfd.fd = fd;
pfd.events = POLLPRI;//POLLIN; //|POLLIN;//type of event to be monitored, POLLPRI  for urgent data, POLLIN to read operations, POLLET edge triggered   
lseek(fd, 0, SEEK_SET); // consume any prior interrupt
read(fd, buf, sizeof buf);

while(choice<=arg)
{ 
poll(&pfd, 1, -1); //wait for interrupt //do {resp=poll(&pfd, 1, -1); }while(resp<0); resp=poll(&pfd, 1, -1); if(resp=poll(&pfd, 1, -1)>=0 ) printf("Poll response is %d\n", resp); else if(resp==-1 ) fprintf(stderr, "Poll response is %d Error\n", resp); 	
lseek(fd, 0, SEEK_SET); // consume interrupt  // to the beginning of the sysfs file and read the new value (or close the file and re-open it to read the value).
read(fd, buf, sizeof buf);//bytes=read(fd, buf, sizeof buf); buf[bytes]=0; sscanf(buf,"%d",&bytes); //if(bytes=read(fd, buf, sizeof(buf))==-1) fprintf(stderr, "Failed to read.\n"); //printf("Bytes read: %d\n",bytes);

write(fdout, buf, sizeof(buf)); //set the VALUE
//printf("\rCycle:%u Input now after interrupt has happened is %d",choice, atoi(buf)); //itan APENERGOPOIIMENO  //printf("bytes = %d ",bytes);  
//printf("\nCycle:%u Input now after interrupt has happened is %d",choice-1, atoi(buf)); //%s %d",choice-1, buf, atoi(buf));   
choice++; 
 } // end of WHILE
